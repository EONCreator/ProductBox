...
namespace ProductBox.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        [HttpPut("edit/{id}")]
        public async Task<IActionResult> Put([FromRoute] int id, [FromBody] Product values) {
            var model = await _context.Products.FirstOrDefaultAsync(item => item.Id == id);

            var modelValues = JsonConvert.SerializeObject(values);
            var valuesDict = JsonConvert.DeserializeObject<IDictionary>(modelValues);
